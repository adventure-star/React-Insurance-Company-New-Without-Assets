...
import QuotationSelectMoto from '../QuotationSelectMoto';


const SlideShowCarouselMoto = (props) => {
                    ...
                    <div className={`${peritem === 2 ? `max-w-1000` : ``} ${peritem === 3 ? `max-w-1296` : ``} mx-auto px-8 sm:px-12 xl:px-24 1xl:px-32`}>
                        <Slider {...settings} slidesToShow={peritem} afterChange={afterChange} className={`${peritem === 3 ? 'bg-white rounded-2xl' : ``} `}>
                            {props.quotes.map((quote => (
                                (
                                    <QuotationSelectMoto key={quote.id} best={quote.isHighlighted} data={quote} peritem={peritem} goHiringPage={(id) => props.goHiringPage(id)} />
                                )
                            )))}
                        </Slider>
                    </div>
                ...
                <div className="max-w-1000 mb-8 mx-auto px-8 sm:px-12 xl:px-24 1xl:px-32">
                    <div className="w-full flex items-center justify-center">
                        ...
                        {props.quotes.length !== 0 &&
                            <QuotationSelectMoto best={props.quotes[0].isHighlighted} data={props.quotes[0]} peritem={peritem} goHiringPage={(id) => props.goHiringPage(id)} />
                        }
                    </div>
                </div>
...
export default SlideShowCarouselMoto;
