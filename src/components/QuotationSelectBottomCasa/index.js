...
import { formatNumber, insurerLogo, randomColor } from '../../services/utils';
import QuotationDetailsCasa from '../QuotationDetailsCasa';


const QuotationSelectBottomCasa = (props) => {
            ...
            <div className={`w-full px-8 ${props.best ? `bg-pink9 ${(props.equal || props.original === 1) ? `` : `absolute`} ${(props.option === 1 || props.option === 2) ? `top-minus36` : ``}  ${props.option === 3 ? `top-minus72` : ``} shadow-xl rounded-tl-3xl rounded-tr-3xl` : `bg-white rounded-3xl`}`}>
                ...
                {props.open &&
                    <div className={`w-full relative pt-6 pb-12`}>
                        ...
                        <QuotationDetailsCasa best={props.best} coverageid={props.data.coverage.id} />
                    </div>
                }
...
export default QuotationSelectBottomCasa;
