...
import pedi from '../../img/pedi.png';
import rappi from '../../img/rappi.png';
import loadingimg from '../../img/loading.gif';

import StepMarker from '../../components/StepMarker';
import StepItemMoto from '../../components/StepItemMoto';
import ModelItemMoto from '../../components/ModelItemMoto';
...
import {
    ...
    apiGetLeadData,
    apiGetBrandsMoto,
    apiGetGroupsByBrandNameMoto,
    apiGetYearsByBrandIdMoto,
    apiGetModelsByBrandGroupYearMoto,
...
const MotoSteps = (props) => {
                ...
                .then(res => {
                    ...
                    apiGetModelByCodiaMoto(res.data.code)
                        .then(res => {
                            ...
                            setVersion(removeSeveralSpace(res.data.description));
                        })
                        .catch(err => {
                            console.log("err-----", err);
                            setLoadingCodia(false);
    ...
    const getGroups = (brand_ = false) => {
        ...
        return apiGetGroupsByBrandNameMoto({ brand: brand_ ? brand_ : make })
            .then(res => {
    ...
    const getYearsByBrandId = (brand_ = false) => {
        setLoadingYear(true);
        return apiGetYearsByBrandIdMoto({ brand_id: brand_ ? brand_ : makelist.filter(i => i.name === make)[0].id })
            .then(res => {
    ...
    const getModelsByBrandGroupYear = (brand_ = false, group_ = false, year_ = false) => {
        ...
        return apiGetModelsByBrandGroupYearMoto({ brand_id: brand_ ? brand_ : makelist.filter(i => i.name === make)[0].id, group_id: group_ ? group_ : grouplist.filter(i => i.name === group)[0].id, year: year_ ? year_ : year })
            .then(res => {
    ...
    useEffect(() => {
            ...
            if (data.state && data.state.brand && !!make) {
                apiGetBrandsMoto()
                    ...
                    .then(res => {
                        ...
                        console.log("statebrand-----", data.state.brand)
                        apiGetYearsByBrandIdMoto({ brand_id: res.filter(i => i.name === data.state.brand)[0].id })
                            .then(res => {
                                console.log("years-----", res);
                                setLoading3B(false);
                                setYearList(res);
        ...
        if (step === 4) {
            if (data.state && data.state.brand && !!make && !!group && !!year) {
                setLoading4A(true);
                setLoading4B(true);
                apiGetGroupsByBrandNameMoto({ brand: make })
                    .then(res => {
                        console.log("groups-----", res);
                        apiGetModelsByBrandGroupYearMoto({ brand_id: makelist.filter(i => i.name === make)[0].id, group_id: res.filter(i => i.name === group)[0].id, year: year })
                            .then(res => {
                                console.log("models-----", res);
                                setVersionList(res);
                                setLoading4B(false);
    ...
    const sendData = e => {
        ...
        if (checkValidation()) {
            ...
            if (leadID && vehicleID) {
                ...
                apiGetModelByCodiaMoto(versionid)
                    .then(res => {
                    ...
                    <div className="w-full max-w-778 mr-auto">
                        <div className="w-full pl-8 pt-15">
                            ...
                            <div id="modal-wrapper" className="pt-10 pl-2 sm:pl-8 md:pl-0 lg:pl-16 pb-8 sm:pb-40">
                                {step === 1 &&
                                    <>
                                        <p className="text-black font-SpartanBold text-xl md:text-2xl pb-8 pr-4">Seleccioná la marca de tu moto</p>
                                        <div className="pr-8 sm:pr-16 md:pr-8 lg:pr-32 1xl:pr-64">
                                            ...
                                            <div className="w-full grid grid-cols-2 sm:grid-cols-3 col-gap-8 md:col-gap-4 1xl:col-gap-8 row-gap-8 md:row-gap-4 1xl:row-gap-8 pt-8 pb-3">
                                                {makelistwithimage.filter(i => i.name.toLocaleLowerCase().includes(makefilter.toLocaleLowerCase())).map(item => (
                                                    <StepItemMoto key={item.id} name={item.name} image={item.image} item={item.name} state={make} onClick={() => { setMake(item.name); setTimeout(function () { setStep(2) }, 500); }} />
                                                ))}
                                                ...
                                                <div className="w-full mt-2 py-2 pl-4 pr-3 bg-white rounded-xl">
                                                    <div className="w-full model-select bg-white pb-4" style={{ maxHeight: "456px" }}>
                                                        ...
                                                        {grouplist.filter(i => i.name.toLocaleLowerCase().includes(modelfilter.toLocaleLowerCase())).map(item => (
                                                            <ModelItemMoto key={item.id} name={item.name} item={item.name} group={group} onClick={() => { setGroup(item.name); setTimeout(function () { setStep(3) }, 500) }} />
                                                        ))}
                                            ...
                                            <div className="w-full model-select mt-2 rounded-xl pt-6 pb-4">
                                                ...
                                                <div className="w-full grid grid-cols-2 sm:grid-cols-4 col-gap-4 row-gap-4">
                                                    {yearlist.filter(i => i.toString().includes(yearfilter.toLocaleLowerCase())).filter(i => yearstate === 0 ? i >= 2007 : i < 2007).map(item => (
                                                        <p key={item} className={`${item === year ? `text-black bg-pink2 shadow-lg font-OpenSansBold` : `text-gray1 bg-white font-OpenSansRegular`} text-lg md:text-lg px-4 py-3 rounded-xl text-center sm:text-left cursor-pointer`} onClick={() => handleYear(item)}>{item}</p>
                                                    ))}
                                                </div>
                                ...
                                {step === 4 &&
                                    <>
                                        <p className="text-black font-SpartanBold text-xl md:text-2xl pb-8 pr-4">Seleccioná la versión de tu moto</p>
                                        <div className="pr-8 sm:pr-16 lg:pr-32 1xl:pr-64">
                                            <div className="w-full flex items-center text-center mx-auto">
                                                ...
                                                <div className="w-full mt-2 py-2 pl-4 pr-3 bg-white rounded-xl">
                                                    <div className="w-full model-select bg-white pb-4" style={{ maxHeight: "456px" }}>
                                                        ...
                                                        {versionlist.filter(i => i.name.toLocaleLowerCase().includes(versionfilter.toLocaleLowerCase())).map(item => (
                                                            <ModelItemMoto key={item.id} name={item.name} item={item.name} group={version} onClick={() => { setVersion(item.name); setTimeout(function () { setStep(5) }, 500); }} />
                                                        ))}
                                            ...
                                            <div className="pt-4 flex items-center float-right cursor-pointer" onClick={() => setModalIsOpen(true)}>
                                                <p className="font-OpenSansSemiBold text-xl text-purple">¿Cuál es la versión de mi moto?</p>
                                                <img src={right1} alt="" />
                                            </div>
                                ...
                                {/* {step === 5 &&
                                    <>
                                        <p className="text-black font-SpartanBold text-xl md:text-2xl pb-4 pr-4">¿Tu moto tiene uso profesional?</p>
